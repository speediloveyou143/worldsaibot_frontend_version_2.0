  ...
  async function handleDelete() {
    try {
      const response = await axios.delete(
        `${BACKEND_URL}/delete-roadmap/${data._id}`,
        { withCredentials: true }
      );
      if (response.status === 200) {
        showAlert("Roadmap deleted successfully!", "success");
      } else {
        showAlert("Failed to delete the roadmap.", "error");
      }
    } catch (error) {
    ...
    <div className="card bg-[#16191F] text-white my-2">
      <div className="card-body items-center text-center">
        ...
        <div className="card-actions justify-end">
          <Link to={`/admin-dashboard/profile/update-road-map/${data._id}`}>
...
function AllRoadMaps() {
  ...
  async function fetchRoadMap() {
    try {
      const response = await axios.get(`${BACKEND_URL}/show-roadmaps`, {
        withCredentials: true,
      });
  ...
  return (
    <div className="flex flex-wrap align-center justify-evenly">
      {roadMap.map((x, index) => (
        <RoadMapCard key={index} data={x} showAlert={showAlert} />
      ))}
      {alert && (
        <div
