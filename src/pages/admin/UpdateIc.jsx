...
const UpdateIc = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      try {
        // Use dedicated /update-ic endpoint
        const response = await api.put(`/update-ic/${id}`, { iCertificates });
        if (response.status === 200) {
  ...
  return (
    <div className="h-full w-full overflow-auto p-6">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-white mb-2">Update Internship Certificates</h1>
        <p className="text-slate-400">Manage student internship certificates</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        {iCertificates.map((cert, index) => (
