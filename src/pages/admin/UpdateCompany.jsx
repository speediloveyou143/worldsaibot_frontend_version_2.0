...
function UpdateCompany() {
      ...
      try {
        ...
        if (response.status === 200) {
          setFormData({
            ...
            logo: response.data.logoUrl || response.data.logo || ""
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const response = await APIService.companies.update(id, {
        companyName: formData.companyName,
        logo: formData.logo  // Backend /update-company expects "logo" field
      });

      if (response.status === 200) {
        showAlert("Company updated successfully!", "success");
        setTimeout(() => {
          navigate("/admin-dashboard/profile/all-company-logos");
        }, 2000);
      }
    } catch (err) {
  ...
  return (
    <div className="h-full w-full overflow-auto p-6">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-white mb-2">Update Company Logo</h1>
        <p className="text-slate-400">Modify company logo details</p>
      </div>
